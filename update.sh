...
# Channel keepalive-probe unit install (Linux only). The token-free IDLE-path
# keepalive producer (scripts/channel-keepalive-probe.sh) shipped as a unit
# TEMPLATE in scripts/systemd/ but was never generated by install-linux.sh, so
# on every machine installed before that fix NOTHING advances
# store/.channel-keepalive except organic inbound traffic. The consequence is
# not cosmetic: once a quiet window passes channel-monitor's 45-min
# liveness-trust ceiling, the watchdog respawn-panes a healthy session, and
# because a respawn does not touch the file either, it repeats every 15 min (the
# respawn grace) until the operator writes something. Measured on the reference
# host 2026-09-09: 5 respawns in one silent hour, each one destroying the running
# conversation. Idempotent: writes only if the unit is absent, and only arms the
# timer if it is not already enabled.
install_keepalive_probe_unit() {
  units_dir="${1:-$HOME/.config/systemd/user}"
  # macOS has its own installer (scripts/install-channel-keepalive-probe.sh);
  # nothing to do where there is no user systemd.
  [ "$(uname -s)" = "Linux" ] || return 0
  command -v systemctl >/dev/null 2>&1 || return 0
  probe="$INSTALL_DIR/scripts/channel-keepalive-probe.sh"
  [ -f "$probe" ] || return 0
  mkdir -p "$units_dir" 2>/dev/null || return 0

  _written=0
  if [ ! -f "$units_dir/channel-keepalive-probe.service" ]; then
    cat >"$units_dir/channel-keepalive-probe.service" <<EOF
[Unit]
...
EOF
  ...
  if [ ! -f "$units_dir/channel-keepalive-probe.timer" ]; then
    cat >"$units_dir/channel-keepalive-probe.timer" <<EOF
[Unit]
...
EOF
  ...
  # enable --now, not enable: a bare enable defers the first tick to the next
  # boot, and the false-respawn loop runs for the whole interval in between.
  if ! systemctl --user is-enabled channel-keepalive-probe.timer >/dev/null 2>&1; then
    if systemctl --user enable --now channel-keepalive-probe.timer 2>/dev/null; then
      echo -e "  Csatorna keep-alive szonda telepitve es elinditva (channel-keepalive-probe.timer)"
    else
      echo -e "  FIGYELEM: a keep-alive szonda unitja elkeszult, de az engedelyezese nem sikerult -- a csendes idoszakok tovabbra is fals respawnt valthatnak ki."
    fi
  elif ! systemctl --user is-active channel-keepalive-probe.timer >/dev/null 2>&1; then

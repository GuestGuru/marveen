...
- **`CHANNEL_PLUGINS_EXTRA` a BÁZIS providert ismétli (Marveen .env, 2026-08-03).** A `channels.sh` a `--channels` listát így építi: `plugin:$PLUGIN_ID` (a `CHANNEL_PROVIDER`-ből) + minden `CHANNEL_PLUGINS_EXTRA` elem `plugin:`-prefixszel. Az EXTRA a TÖBBI provider co-listen slotja (pl. discord), NEM a bázisé. Ha ugyanaz áll benne (`CHANNEL_PROVIDER=telegram` + `CHANNEL_PLUGINS_EXTRA=telegram@claude-plugins-official`), a parancssor `--channels plugin:telegram@... plugin:telegram@...` lesz -- a duplikáció önmagában nem dob hibát, ezért NÉMA. Két következménye van: (a) a második provider (discord) sosem indul co-listen módban, tehát az inbound ott senkihez nem jut el, miközben a bot online és a plugin MCP toolok elérhetők -> "van bot, van párosított DM, de senki nem olvassa"; (b) ugyanaz a plugin kétszer szerepel egy session `--channels` listájában. Diagnózis: `tr '\0' ' ' < /proc/<pid>/cmdline` és nézd, hogy a `plugin:` tokenek KÜLÖNBÖZŐEK-e. Javítás: az EXTRA-ba a MÁSIK provider kerüljön (vagy üres legyen), és a plugin `enabledPlugins`-ben is `true` legyen -- de a változás csak ÚJRAINDÍTÁS után él, tehát ne csendben told rá egy futó ágensre.

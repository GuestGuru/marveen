...
- **A marveen boxon (2026-08-02) NINCS SEMMILYEN GitHub hitelesítés, tehát `git push` NEM megy — sem neked, sem a gazdának.** Ellenőrzött állapot: `git config --get credential.helper` üres, `~/.git-credentials` nem létezik, `GH_TOKEN`/`GITHUB_TOKEN` nincs, a `gh` telepítve van de "not logged into any GitHub hosts". Az alábbi `gh`-alapú eljárás ezért ezen a gépen **nem futtatható**. Amit a bot használhat: a gg-mcp `github_*` tooljai — de a `github_request` CSAK olvas, a `github_open_pr` pedig CSAK már létező remote ághoz tud PR-t nyitni. Ág létrehozására és commit feltolására nincs eszköz, ezért a lánc az első lépésnél megáll. Nyitott kérés: **IT-461**. Amíg nincs megoldva: kommitolj lokálisan feature ágra, állítsd vissza a checkoutot `main`-re (hogy a munkafa tiszta legyen, lásd lent), és a push-t kérd embertől (`gh auth login`, majd `git push -u origin <ág>`).
- **A piszkos munkafa MEGBÉNÍTJA a frissítést.** A `src/update-preflight.ts` `dirty-tree` ága bármilyen el nem kommitolt tracked változásnál megtagadja az update-et (egyetlen kivétel a `HEARTBEAT.md`), a `local-commits` ág pedig akkor, ha a lokális ág az upstream elé került. Ezért soha ne hagyj lokális foltozást a checkoutban: vagy visszavezeted (feature ág → develop → main), vagy eldobod.

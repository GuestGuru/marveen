...
MISSING_PKGS=""
# GG fork: a sqlite3 CLI kulon csomag: a better-sqlite3 npm modul (amit lentebb
# forditunk) NEM hozza magaval. A futasido tobb helyen hivja -- doctor.sh,
# scripts/status.ts, scripts/migrate-main-agent-id.sh, a backup WAL-checkpointja, es a
# seed-skillek (dream-engine, kanban-audit, handoff) sqlite3-parancsokat irnak elo az
# agentnek. Nelkule ezek nem hibaznak hangosan, hanem elnemulnak: a doctor "? memories"-t ir.
for pkg in ffmpeg git tmux lsof curl python3 pipx unzip zstd sqlite3; do

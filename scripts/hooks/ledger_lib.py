...
def agent_id_from_cwd(cwd):
    ...
    ⚠️ GG fork -- a MERT incidens, ami miatt a basename-tipp kikerult:
    2026-08-26 12:31-kor a `tool_call_log`-ba ket sor keruelt
    `agent_id="fo-agens-restart-kontextus"` ertekkel -- egy SKILL neve --, mert a
    fordulo epp abba a konyvtarba cd-zett. Az upstream javitasa (env-var, majd
    fo-agens) ezt lezarja; a bizonyitek itt marad, hogy ne kerüljön vissza.
    """
    cwd = (cwd or "").rstrip("/")
    install = _install_dir().rstrip("/")
    agents_root = os.path.join(install, "agents")

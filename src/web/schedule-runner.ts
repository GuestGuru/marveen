...
//             further OWNER_ESCALATION_EXTRA_MS has elapsed with the session
//             still busy. Keyed off the entry's own clock, not off the stage-1
//             send succeeding, so a failed stage-1 notice (e.g. a DB hiccup)
//             can never silently suppress the stage-2 escalation.
//             GG fork: that clock is `stalledSince` (last visible pane
//             movement), not `injectedAt` -- a task that is still producing
//             output is working, not stuck, at either stage.
...
export function decideTaskTimeout(
  entry: Pick<TaskInflightEntry, 'injectedAt' | 'alerted' | 'ownerAlerted' | 'sawTurn'> & { stalledSince?: number },
  ...
  // GG fork: both stages run off the FROZEN clock, not the elapsed one -- a pane
  // that is still moving is working, not stuck, at either stage.
  const stalledFor = now - (entry.stalledSince ?? entry.injectedAt)
  if (!entry.alerted) {
    if (stalledFor >= opts.timeoutMs) return 'alert'
    return 'hold'
  }
  if (!entry.ownerAlerted && stalledFor >= opts.timeoutMs + opts.ownerExtraMs) return 'escalate'
  return 'hold'
}
...
export function startScheduleRunner(): NodeJS.Timeout {
  ...
  async function runCheck() {
    ...
    for (const [key, entry] of taskInflightMap) {
      ...
      // upstream: record evidence that the injection actually started a turn.
      // 'busy' is the direct observation; the transcript mtime covers the task
      // that ran and finished entirely between two sweeps, which no pane sample
      // would ever catch. Deliberately NOT set on 'typing': that state means our

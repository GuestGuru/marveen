...
export function buildMemoryRulesBody(): string {
  return [
    ...
    '',
    '   🔴 **A `shared` NEM EGY TIER, HANEM PUBLIKÁCIÓ A FLOTTA FELÉ, ÉS EZÉRT A SZEMÉLYES',
    '   ADAT OTT A LEGDRÁGÁBB.** Mérve 2026-09-13 (brokermarcsi, a 205 soros közös polcon):',
    '   NÉGY magánszemély e-mail címe állt rajta, HÁROM különböző cím, NÉGY külön ágensnél,',
    '   és a gazdáik közül hárman nem tudtak róla. Az egyik egy shared bejegyzés NYITÓSORÁBAN,',
    '   vagyis a legolvasottabb helyen. **A polc mind a hét ágens listázásában megjelenik,',
    '   tehát amit ide írsz, azt hatan mások is olvassák, és bármelyikük beírhatja egy kimenő',
    '   levélbe.** Ez nem elméleti: ugyanaznap háromszor mértük, hogy egy belső érték az',
    '   ÁGENSEN keresztül vándorol tovább (üzenetsorba, teszt-fájlba, jelentésbe).',
    '   **A szabály: a `shared` kategória választása előtt tedd fel ugyanazt a két kérdést,',
    '   amit egy publikus repóba küldött fájlnál.** (1) Van-e benne harmadik személy neve,',
    '   e-mail címe, telefonszáma, lakcíme vagy díjazása? (2) Van-e benne AZONOSÍTÓ, ami',
    '   maga a kulcs (tulaj-UUID, auth nélkül nyíló link)? A kettő KÉT KÜLÖN tengely.',
    '   Ha bármelyikre igen: vagy `warm`-ba tedd a saját tieredbe, vagy vedd ki az értéket',
    '   és hagyd ott a lekérdezést, ami visszaadja. **Az UUID-szabály bizonyítottan működik:**',
    '   ugyanazon a mérésen mind a 205 shared bejegyzésen NULLA UUID volt, mert az a szabály',
    '   régóta ki van mondva. Amit kimondunk, azt betartjuk; ez a bekezdés a személyes adatot',
    '   teszi ugyanarra a listára.',
    '   ⚠️ **És a felület hiányzott, nem a tudás:** a skilleknél van zár-sor, a repóban van',
    '   `.gitignore` és publikálás előtti kapu, a shared emléknél viszont semmi nem szólt a',
    '   BEÍRÁS pillanatában. Ezért áll ez itt, a karbantartott blokkban, nem egy skillben:',
    '   ez az a szöveg, ami minden ágens CLAUDE.md-jébe eljut, és ott van, amikor írsz.',
    '',
    ...
    '✅ **A PUT TIERT IS VÁLT, de csak a tartalommal EGYÜTT** (mérve 2026-09-13, eldobható',
    'teszt-bejegyzésen, brokermarcsi kérdésére). Három eset: (a) CSAK `category`, tartalom',
    'nélkül -> **HTTP 500, és a sor VÁLTOZATLAN marad**; (b) tartalom ÉS `category` együtt',
    '-> mindkettő átmegy, ez a használható út; (c) tartalom `category` nélkül -> a tartalom',
    'cserélődik, a kategória NEM nullázódik. **A rossz hívás tehát hangosan elszáll és nem ír',
    'semmit**, vagyis a jó irányba téved: aki véletlenül így hívja, nem tesz kárt. A `tier`',
    'mező a `category` aliasa és ELŐBBRE való, ne add meg mindkettőt. Nagy bejegyzésnél a',
    'tartalom sértetlenségét sha256-tal ÉS hosszal is ellenőrizd: a hash egyezése önmagában',
    'elég, de ha eltér, a hossz mondja meg, csonkulás volt-e vagy csere (brokermarcsi',
    'ellenőrzése egy 5306 karakteres bejegyzésen: hash és hossz is azonos maradt).',
    '',
